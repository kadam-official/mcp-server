...
describe("resource cabinet scoping", () => {
  ...
  it("pub session lists publisher resources and no advertiser ones", async () => {
    const uris = await listUris("pub", { pubKey: "b" });
    expect(uris).toContain("kadam://reference/site-states");
    expect(uris).toContain("kadam://reference/ad-unit-types");
